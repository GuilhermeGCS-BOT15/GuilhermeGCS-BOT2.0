...
const wait = async (media) => new Promise(async (resolve, reject) => {
    ...
    try {
    	...
    	let belief = () => similarity < 0.89 ? "não acredito muito : " : ""
...
~> Estação  : *${season}*`});
		...
		console.log(e)
		reject(`desculpe, não consigui achar esse anime`)
	}
})
...
const simih = async (text) => {
	try {
		...
		return 'Simi não sabe do que está falando, tente falar de uma forma mais detalhada'
	}
}
...
const banner = cfonts.render(('GUILHERMEGCS|BOT'), {
    font: 'chrome',
    color: 'candy',
    align: 'center',
    gradient: ["red","yellow"],
